...
The first time a route activates, macOS asks for **System Audio Recording** permission (Settings → Privacy & Security). This is required — process taps are technically an audio-capture facility, even though AudioRouter only re-renders the audio and never records it.

## How it works

For each routed app that has a live audio presence, AudioRouter:
...
Everything is event-driven via CoreAudio property listeners (device plug/unplug, audio processes appearing/exiting, default-device changes) — no polling, and zero audio I/O when no routes are active. Rules persist in `~/Library/Application Support/AudioRouter/rules.json` keyed by bundle ID, and re-apply automatically when apps relaunch or devices reappear.
...
- Quitting AudioRouter removes all taps, so apps immediately revert to their normal output.
